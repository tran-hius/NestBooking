...
export function setupSwaggerUi(app: express.Application): void {
  const readSwaggerDoc = (moduleName: string) => {
    return JSON.parse(
      fs.readFileSync(
        path.join(__dirname, "..", "modules", moduleName, "docs", `swagger-${moduleName}.json`),
        "utf8"
      )
    );
  };
