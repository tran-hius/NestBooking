...
## 1. Mục Tiêu (Purpose)
* **AgentService** chịu trách nhiệm quản lý toàn bộ vòng đời hồ sơ đối tác (Chủ khách sạn / Merchant) trên hệ thống, từ lúc đăng ký gian hàng, chờ duyệt, cho đến khi được kích hoạt kinh doanh chính thức.
* Dịch vụ này cô lập hoàn toàn logic quản trị hồ sơ đối tác, **không** xử lý nghiệp vụ quản lý khách sạn/phòng (thuộc HotelService), không xử lý xác thực đăng nhập/token (thuộc AuthService).
* AgentService là điều kiện tiên quyết đầu vào cho HotelService: một tài khoản chỉ được phép tạo khách sạn (`UC01` bên HotelService) khi hồ sơ Agent tương ứng đã ở trạng thái `APPROVED`.
...
* **Kết quả:** Hồ sơ đối tác được tạo thành công, chờ Admin xét duyệt. Agent **chưa** có quyền tạo khách sạn ở bước này.
...
* **Kết quả:** Agent chính thức được phép tạo khách sạn thông qua HotelService.
...
* **Nghiệp vụ:** Chuyển `approvalStatus` sang `BANNED`. Khi Agent bị khóa, toàn bộ khách sạn thuộc quyền sở hữu của Agent đó **không** tự động bị xóa, nhưng HotelService cần kiểm tra trạng thái Agent trước khi cho phép các thao tác tạo/sửa khách sạn mới.

### 3.2. Quản Lý Chuỗi Khách Sạn (Multi-Hotel Ownership)

#### UC07 - Xem Danh Sách Khách Sạn Thuộc Sở Hữu
* **Actor:** Agent.
* **Mô tả:** Agent xem danh sách toàn bộ khách sạn mà mình đang sở hữu và quản lý (dữ liệu khách sạn thực tế nằm ở HotelService, AgentService chỉ đóng vai trò cung cấp `userId` để HotelService lọc theo chủ sở hữu).
* **Ghi chú:** Một Agent có thể sở hữu và quản lý nhiều khách sạn khác nhau (mô hình chuỗi).
...
* **BR05:** Chỉ hồ sơ đối tác có `approvalStatus = APPROVED` mới được phép thực hiện các hành vi tạo/sửa khách sạn bên HotelService (`BR04` của HotelService).
...
| `AGENT_NOT_APPROVED` | 403 Forbidden | Hồ sơ đối tác chưa được duyệt (`PENDING`/`REJECTED`), chưa đủ điều kiện tạo khách sạn. |
