...
export class RoomController {
  ...
  updateRoom = async (req: Request, res: Response): Promise<void> => {
    logger.info("[RoomController] Update room", { id: (req.params.id as string) });
    const ownerId = req.user?.userId as string;

    const room = await this.roomService.updateRoom(ownerId, (req.params.id as string), req.body);
    successResponse(res, HttpStatus.OK, "Cập nhật phòng thành công", room);
  };

  deleteRoom = async (req: Request, res: Response): Promise<void> => {
    logger.warn("[RoomController] Delete room", { id: (req.params.id as string) });
    const ownerId = req.user?.userId as string;

    await this.roomService.deleteRoom(ownerId, (req.params.id as string));
