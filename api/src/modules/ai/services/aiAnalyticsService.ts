...
import { IBookingReadRepository } from "@/modules/booking/interfaces";
...
export class AiAnalyticsService {
  ...
  public async analyzeHotelBookings(
    hotelId: string,
    requesterId: string,
  ): Promise<string> {
    if (!this.ai) {
      throw new BadRequestError(
        "Gemini API Key chưa được cấu hình trong file .env",
      );
    }
    ...
    if (hotel.ownerId !== requesterId) {
      throw new ForbiddenError(
        "Bạn không có quyền phân tích dữ liệu khách sạn này",
      );
    }
    ...
    const dataContext = JSON.stringify({
      ...
      confirmedBookings: statsData.filter(
        (b) => b.status === BookingStatus.CONFIRMED,
      ).length,
      checkedInBookings: statsData.filter(
        (b) => b.status === BookingStatus.CHECKED_IN,
      ).length,
      cancelledBookings: statsData.filter(
...
`;
    ...
    try {
      ...
      throw new BadRequestError(
        "Lỗi khi kết nối với AI Model. Vui lòng thử lại sau.",
      );
    }
  }

  public async analyzeAllHotels(requesterId: string): Promise<string> {
    if (!this.ai) {
