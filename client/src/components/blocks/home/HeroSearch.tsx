...
export default function HeroSearch() {
  ...
  const handleSearch = () => {
    if (!location.trim()) {
      toast.error(t("heroSearch.errLocation"));
      return;
    }
    if ((checkIn || checkOut) && (!checkIn || !checkOut || checkOut <= checkIn)) {
      toast.error(t("heroSearch.errDates"));
      return;
    }
  ...
  const formatDateRange = () => {
    ...
    const format = (dateStr: string) => dateStr ? formatter.format(new Date(`${dateStr}T00:00:00`)) : t("heroSearch.selectDate");
    ...
    <section className="relative z-20 flex min-h-[720px] w-full isolate items-center pt-24 md:min-h-[680px] md:pt-20">
      ...
      <div className="container relative z-20 py-14">
        <div className="max-w-4xl">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3.5 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-cyan-100 backdrop-blur-md"><ShieldCheck className="h-4 w-4" />{t("heroSearch.directBooking")}</div>
          <h1 className="max-w-3xl text-4xl font-black leading-[1.08] tracking-tight text-white drop-shadow-lg sm:text-5xl lg:text-6xl">{t("heroSearch.title1")} <span className="text-cyan-300">{t("heroSearch.title2")}</span></h1>
          <p className="mt-5 max-w-2xl text-base leading-relaxed text-blue-50/90 sm:text-lg">{t("heroSearch.subtitle")}</p>

          <div className="mt-8 flex flex-wrap gap-x-5 gap-y-2 text-sm text-white/85"><span className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-cyan-300" />{t("heroSearch.activeData")}</span><span className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-cyan-300" />{t("heroSearch.trackBooking")}</span></div>
          ...
          <div className="relative z-50 mt-9 w-full rounded-[24px] border border-white/20 bg-white/95 p-2.5 shadow-[0_24px_70px_rgba(2,20,45,0.35)] backdrop-blur-xl">
            <div className="mb-2 flex items-center gap-2 px-2 py-1 text-xs font-bold uppercase tracking-[0.13em] text-slate-500"><Building2 className="h-4 w-4 text-primary" />{t("heroSearch.findStay")}</div>
            <div className="relative grid gap-2 lg:grid-cols-[1.25fr_.8fr_1fr_auto]">
              ...
              <button type="button" className="flex w-full min-w-0 items-center gap-3 text-left" onClick={() => setIsCalendarOpen((open) => !open)}><Calendar className="h-5 w-5 shrink-0 text-primary" /><span className="min-w-0"><span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400">{t("heroSearch.stayDates")}</span><span className="block truncate font-semibold text-slate-900">{formatDateRange()}</span></span></button>
            ...
            <div ref={guestsRef} className={`relative flex min-w-0 items-center rounded-2xl border px-4 py-3.5 transition ${isGuestsOpen ? "border-primary bg-blue-50 ring-4 ring-blue-100" : "border-slate-200 bg-slate-50 hover:border-blue-300"}`}>
              <button type="button" className="flex w-full min-w-0 items-center gap-3 text-left" onClick={() => setIsGuestsOpen((open) => !open)}><Users className="h-5 w-5 shrink-0 text-primary" /><span className="min-w-0"><span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400">{t("heroSearch.guestsAndRooms")}</span><span className="block truncate font-semibold text-slate-900">{guests.adults + guests.children} {t("heroSearch.guests")}, {guests.rooms} {t("heroSearch.rooms")}</span></span></button>
              {isGuestsOpen && <GuestsDropdown {...guests} onChange={setGuests} onClose={() => setIsGuestsOpen(false)} />}
            </div>
              <Button type="button" onClick={handleSearch} className="h-auto min-h-14 rounded-2xl bg-[#087fd1] px-7 text-base font-bold text-white shadow-lg shadow-blue-500/20 hover:bg-[#056eb8]"><Search className="h-5 w-5" />{t("heroSearch.search")}</Button>
            </div>
          </div>

          <div className="mt-5 flex items-center gap-2 text-sm text-blue-50/80"><MapPin className="h-4 w-4 text-cyan-300" /><span>{t("heroSearch.popularSuggestions")}</span>{["Hà Nội", "Đà Nẵng", "Hạ Long"].map((item) => <button key={item} type="button" onClick={() => setLocation(item)} className="font-semibold text-white underline-offset-4 hover:underline">{item}</button>)}</div>
        </div>
      </div>
    </section>
  );
