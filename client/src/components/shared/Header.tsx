...
const primaryLinks = [
  { key: "hotels", href: "/search", icon: Search },
      ...
      <div className="container flex h-[72px] items-center justify-between gap-4">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Điều hướng chính">
          {primaryLinks.map((item) => {
            ...
            return <Link key={item.href} to={item.href} className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-sm font-semibold transition ${active ? isHome ? "bg-white/15 text-white" : "bg-blue-50 text-primary" : subtleText}`}><item.icon className="h-4 w-4" />{t(`header.${item.key}`)}</Link>;
          })}
          <Link to="/partner/register" className={`ml-1 inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-sm font-semibold transition ${subtleText}`}><Building2 className="h-4 w-4" />{t("header.partner")}</Link>
        </nav>

        <div className="ml-auto flex shrink-0 items-center gap-2 lg:ml-0">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className={`rounded-xl ${isHome ? "text-white hover:bg-white/10 hover:text-white" : "text-slate-700"}`}>
          ...
          {isAuthenticated ? <AccountMenu isHome={isHome} user={user} onNavigate={navigate} onLogout={handleLogout} t={t} /> : <div className="hidden items-center gap-2 sm:flex"><Button variant="ghost" asChild className={`rounded-xl font-semibold ${isHome ? "text-white hover:bg-white/10 hover:text-white" : "text-slate-700"}`}><Link to="/login"><User className="mr-1.5 h-4 w-4" />{t("header.signIn")}</Link></Button><Button asChild className="rounded-xl px-5 font-bold text-white shadow-lg shadow-blue-500/15"><Link to="/register">{t("header.signUp")}</Link></Button></div>}
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent side="right" className="w-[min(88vw,360px)] border-l-0 bg-[#05285d] p-0 text-white">
              ...
              <div className="flex h-[calc(100%-105px)] flex-col overflow-y-auto p-4">
                <nav className="space-y-2">{primaryLinks.map((item) => <MobileLink key={item.href} href={item.href} icon={item.icon} label={t(`header.${item.key}`)} active={location.pathname === item.href} />)}<MobileLink href="/partner/register" icon={Building2} label={t("header.partner")} active={location.pathname === "/partner/register"} />{isAuthenticated && <><MobileLink href="/my-bookings" icon={CalendarRange} label={t("header.myBookings")} active={location.pathname === "/my-bookings"} /><MobileLink href="/settings/personal-details" icon={UserCircle} label={t("header.manageAccount")} active={location.pathname === "/settings/personal-details"} />{user?.role === "AGENT" && <MobileLink href="/partner/dashboard" icon={LayoutDashboard} label={t("header.partner")} active={location.pathname.startsWith("/partner/")} />}</>}</nav>
                <div className="mt-auto border-t border-white/10 pt-4">{isAuthenticated ? <div className="space-y-3"><div className="flex items-center gap-3 rounded-2xl bg-white/[0.07] p-3"><UserAvatar user={user} /><div className="min-w-0"><div className="truncate text-sm font-bold">{user?.profile?.fullName || "Tài khoản NestBooking"}</div><div className="truncate text-xs text-blue-100/55">{user?.email}</div></div></div><Button variant="outline" onClick={handleLogout} className="w-full rounded-xl border-red-300/20 bg-red-400/10 text-red-200 hover:bg-red-400/20 hover:text-white"><LogOut className="mr-2 h-4 w-4" />{t("header.logout")}</Button></div> : <div className="grid gap-2"><SheetClose asChild><Button variant="outline" asChild className="rounded-xl border-white/20 bg-white/5 text-white hover:bg-white/10 hover:text-white"><Link to="/login"><User className="mr-2 h-4 w-4" />{t("header.signIn")}</Link></Button></SheetClose><SheetClose asChild><Button asChild className="rounded-xl bg-white font-bold text-[#05285d] hover:bg-blue-50"><Link to="/register"><UserPlus className="mr-2 h-4 w-4" />{t("header.signUp")}</Link></Button></SheetClose></div>}</div>
              </div>
            </SheetContent>
          </Sheet>
        </div>
