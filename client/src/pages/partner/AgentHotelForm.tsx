      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div className="space-y-6">
          ...
          <FormSection number="03" icon={Clock3} title="Vận hành và tiện nghi" description="Thiết lập thời gian lưu trú và các tiện ích có thật tại chỗ nghỉ.">
            ...
            <div className="space-y-3 md:col-span-2"><Label>Tiện nghi nổi bật</Label><div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-4">{amenityOptions.map((amenity) => { const Icon = amenity.icon; const active = selectedAmenities.includes(amenity.value); return <button key={amenity.value} type="button" onClick={() => toggleAmenity(amenity.value)} className={`flex items-center gap-2.5 rounded-xl border px-3 py-3 text-left text-sm font-medium transition ${active ? "border-sky-400 bg-sky-50 text-sky-800 dark:border-sky-800 dark:bg-sky-950/30 dark:text-cyan-300" : "border-slate-200 bg-white text-slate-600 hover:border-sky-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300"}`}><span className={`flex h-8 w-8 items-center justify-center rounded-lg ${active ? "bg-sky-600 text-white" : "bg-slate-100 text-slate-500 dark:bg-zinc-800"}`}><Icon className="h-4 w-4" /></span>{amenity.label}{active && <Check className="ml-auto h-4 w-4" />}</button>; })}</div></div>
...
function ChecklistItem({ done, label }: { done: boolean; label: string }) { return <div className={`flex items-center gap-2.5 ${done ? "text-slate-700 dark:text-zinc-200" : "text-slate-400"}`}><span className={`flex h-5 w-5 items-center justify-center rounded-full ${done ? "bg-sky-100 text-sky-700 dark:bg-sky-950 dark:text-cyan-300" : "bg-slate-100 dark:bg-zinc-800"}`}>{done && <Check className="h-3.5 w-3.5" />}</span>{label}</div>; }
