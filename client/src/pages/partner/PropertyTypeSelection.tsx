  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 pb-8">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="hidden items-center gap-2 text-xs font-semibold text-slate-400 sm:flex"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-600 text-white">1</span><span className="text-emerald-700 dark:text-emerald-400">Loại hình</span><span className="h-px w-8 bg-slate-200 dark:bg-zinc-700" /><span className="flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 dark:bg-zinc-800">2</span><span>Thông tin</span></div>
      </div>

      <section className="relative overflow-hidden rounded-[28px] bg-[#063a55] px-6 py-8 text-white shadow-[0_18px_50px_rgba(6,58,85,0.2)] md:px-9 md:py-10">
        <div className="absolute -right-12 -top-20 h-72 w-72 rounded-full bg-emerald-400/20 blur-3xl" />
        <div className="relative max-w-3xl"><div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.15em] text-emerald-100"><Building2 className="h-4 w-4" />Bước 1 trong 2</div><h1 className="text-3xl font-bold tracking-tight md:text-4xl">Bạn đang muốn đăng loại chỗ nghỉ nào?</h1><p className="mt-3 max-w-2xl text-sm leading-relaxed text-cyan-50/80 md:text-base">Chọn loại hình mô tả đúng nhất cơ sở của bạn. Thông tin này giúp khách hiểu rõ trải nghiệm lưu trú và có thể thay đổi ở bước tiếp theo.</p></div>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {propertyTypes.map((property) => {
            ...
            return <button key={property.id} type="button" onClick={() => setSelected(property.id)} aria-pressed={isSelected} className={`group relative min-h-52 rounded-2xl border p-5 text-left outline-none transition-all focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 ${isSelected ? "border-emerald-500 bg-emerald-50 shadow-[0_12px_35px_rgba(5,150,105,0.14)] dark:bg-emerald-950/25" : "border-slate-200 bg-white shadow-sm hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-emerald-800"}`}><div className="flex items-start justify-between gap-3"><span className={`flex h-12 w-12 items-center justify-center rounded-2xl transition-colors ${isSelected ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-600 group-hover:bg-emerald-100 group-hover:text-emerald-700 dark:bg-zinc-800 dark:text-zinc-300"}`}><Icon className="h-6 w-6" /></span>{isSelected && <span className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-600 text-white"><Check className="h-4 w-4" /></span>}</div><h2 className="mt-5 text-lg font-bold text-slate-900 dark:text-white">{property.name}</h2><p className="mt-2 text-sm leading-relaxed text-slate-500 dark:text-zinc-400">{property.description}</p><div className={`mt-4 text-xs font-semibold ${isSelected ? "text-emerald-700 dark:text-emerald-400" : "text-slate-400"}`}>{property.hint}</div></button>;
          })}
        </div>

        <aside className="xl:sticky xl:top-24 xl:self-start">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"><div className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Lựa chọn của bạn</div>{selectedProperty ? <div className="mt-4"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300"><selectedProperty.icon className="h-5 w-5" /></span><div><div className="font-bold text-slate-900 dark:text-white">{selectedProperty.name}</div><div className="text-xs text-muted-foreground">{selectedProperty.hint}</div></div></div><p className="mt-4 text-sm leading-relaxed text-slate-500 dark:text-zinc-400">Tiếp theo bạn sẽ nhập tên, địa chỉ, liên hệ, tiện nghi và hình ảnh chỗ nghỉ.</p></div> : <div className="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-7 text-center dark:border-zinc-700 dark:bg-zinc-950"><Building2 className="mx-auto h-7 w-7 text-slate-300" /><p className="mt-2 text-sm text-muted-foreground">Chọn một loại hình để tiếp tục.</p></div>}<Button className="mt-5 h-11 w-full rounded-xl bg-emerald-600 font-bold hover:bg-emerald-700" disabled={!selected} onClick={() => selected && navigate(`/partner/hotels/new?type=${selected}`)}>Tiếp tục nhập thông tin<ArrowRight className="ml-2 h-4 w-4" /></Button><p className="mt-3 text-center text-[11px] leading-relaxed text-slate-400">Chỗ nghỉ mới sẽ được gửi Admin kiểm duyệt trước khi hiển thị công khai.</p></div>
        </aside>
      </div>
    </div>
  );
