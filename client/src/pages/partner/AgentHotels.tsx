...
const statusConfig: Record<HotelStatus, { label: string; className: string; dot: string }> = {
  ...
  ACTIVE: { label: "Đang hoạt động", className: "border-sky-200 bg-sky-50 text-sky-700 dark:border-sky-900 dark:bg-sky-950/40 dark:text-cyan-300", dot: "bg-sky-500" },
...
export default function AgentHotels() {
  ...
  return (
    <div className="space-y-6 pb-8">
      <section className="relative overflow-hidden rounded-[26px] border border-sky-100 bg-gradient-to-br from-white via-sky-50/50 to-cyan-50 p-6 shadow-sm dark:border-zinc-800 dark:from-zinc-900 dark:via-zinc-900 dark:to-sky-950/30 md:p-7">
        <div className="absolute -right-12 -top-20 h-64 w-64 rounded-full bg-cyan-300/20 blur-3xl" />
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div>
            <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-sky-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-sky-700 dark:bg-sky-950 dark:text-cyan-300"><Building2 className="h-4 w-4" />Property workspace</div>
          ...
          <div className="flex flex-wrap gap-2.5"><Button variant="outline" onClick={() => void loadHotels()} disabled={loading} className="h-10 gap-2 rounded-xl bg-white dark:bg-zinc-900"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />Làm mới</Button><Button asChild className="h-10 rounded-xl bg-sky-600 hover:bg-sky-700"><Link to="/partner/property-type"><Plus className="mr-1.5 h-4 w-4" />Thêm chỗ nghỉ</Link></Button></div>
        </div>
      </section>
...
function StatCard({ icon: Icon, label, value, hint, tone }: { icon: LucideIcon; label: string; value: number; hint: string; tone: "blue" | "sky" | "amber" | "violet" }) {
  const tones = { blue: "bg-blue-50 text-blue-700 dark:bg-blue-950/50 dark:text-blue-300", sky: "bg-sky-50 text-sky-700 dark:bg-sky-950/50 dark:text-cyan-300", amber: "bg-amber-50 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300", violet: "bg-violet-50 text-violet-700 dark:bg-violet-950/50 dark:text-violet-300" };
...
function QuickGuide({ number, title, description }: { number: string; title: string; description: string }) {
  return <div className="flex gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"><span className="text-2xl font-black text-sky-200 dark:text-sky-900">{number}</span><div><div className="font-semibold text-slate-900 dark:text-white">{title}</div><p className="mt-1 text-sm leading-relaxed text-muted-foreground">{description}</p></div></div>;
}
