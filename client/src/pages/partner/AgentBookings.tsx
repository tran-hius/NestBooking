...
import type { Booking, BookingStatus, Hotel, PaymentStatus } from "@/types";
...
const statusConfig: Record<BookingStatus, { label: string; className: string; dot: string }> = {
  ...
  COMPLETED: { label: "Hoàn thành", className: "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-300", dot: "bg-emerald-500" },
};
...
const paymentConfig: Record<PaymentStatus, { label: string; className: string }> = {
  ...
  PAID: { label: "Đã thanh toán", className: "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-300" },
...
export default function AgentBookings() {
  ...
  return (
    <div className="space-y-6 pb-8">
      <section className="relative overflow-hidden rounded-[26px] border border-blue-100 bg-gradient-to-br from-white via-blue-50/50 to-emerald-50 p-6 shadow-sm dark:border-zinc-800 dark:from-zinc-900 dark:via-zinc-900 dark:to-blue-950/30 md:p-7">
...
function StatCard({ icon: Icon, label, value, hint, tone }: { icon: LucideIcon; label: string; value: string; hint: string; tone: "blue" | "amber" | "indigo" | "emerald" }) {
  const tones = { blue: "bg-blue-50 text-blue-700 dark:bg-blue-950/50 dark:text-blue-300", amber: "bg-amber-50 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300", indigo: "bg-indigo-50 text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300", emerald: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300" };
...
function StayDates({ booking }: { booking: Booking }) { return <div className="min-w-36"><div className="flex items-center gap-2 text-sm font-medium"><span className="h-2 w-2 rounded-full bg-emerald-500" />{formatDate(booking.checkInDate)}</div><div className="ml-[3px] h-3 border-l border-dashed border-slate-300" /><div className="flex items-center gap-2 text-sm text-slate-500"><span className="h-2 w-2 rounded-full border-2 border-slate-400 bg-white dark:bg-zinc-900" />{formatDate(booking.checkOutDate)}</div><div className="mt-1 text-xs text-muted-foreground">{getNights(booking)} đêm</div></div>; }
function SummaryCard({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) { return <div className="rounded-xl border border-slate-200 bg-slate-50 p-3.5 dark:border-zinc-800 dark:bg-zinc-900"><Icon className="mb-2 h-4 w-4 text-emerald-600" /><div className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">{label}</div><div className="mt-1 line-clamp-2 text-sm font-medium">{value}</div></div>; }
